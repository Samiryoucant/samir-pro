      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <div className={`text-4xl font-black ${isPizza ? 'text-pizza-600' : 'text-lemon-600'}`}>{s.value}</div>
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <button onClick={openModal} className={`px-4 py-2 rounded-lg font-bold text-white flex items-center gap-2 ${isPizza ? 'bg-pizza-600' : 'bg-lemon-600'}`}>
        ...
        <div className="grid grid-cols-1 gap-4">
          {courses.map(c => (
            <div key={c.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
               ...
               <div className="flex gap-2">
                 <button onClick={() => handleEditCourse(c)} className="text-blue-500 hover:bg-blue-50 p-2 rounded-lg transition" title="Edit Course">
                   <Pencil size={18} />
                 </button>
                 <button onClick={() => handleDeleteCourse(c.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg transition" title="Delete Course">
                   <Trash size={18} />
                 </button>
               </div>
            </div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden m-4 flex flex-col max-h-[90vh]">
               {/* Modal Header */}
               <div className="p-4 border-b flex justify-between items-center bg-gray-50">
                  <h3 className="font-bold text-lg">{isEditing ? 'Edit Course' : 'Create New Course'}</h3>
               ...
               <div className="flex border-b">
                 {['basic', 'content', 'images'].map(tab => (
                   <button 
                    ...
                    className={`flex-1 py-4 font-bold text-sm uppercase tracking-wider transition ${activeTab === tab ? (isPizza ? 'bg-pizza-50 text-pizza-600 border-b-2 border-pizza-600' : 'bg-lemon-50 text-lemon-600 border-b-2 border-lemon-600') : 'text-gray-400 hover:bg-gray-50'}`}
                   >
                     {tab}
                   </button>
                 ))}
                   ...
                   <div className="space-y-4">
                     <div>
                       ...
                       <input 
                         ...
                         className="w-full p-3 bg-gray-50 rounded-lg border focus:ring-2 focus:outline-none"
                     ...
                     <div>
                       ...
                       <textarea 
                         ...
                         className="w-full p-3 bg-gray-50 rounded-lg border h-32 focus:ring-2 focus:outline-none"
                   ...
                   <div className="space-y-6">
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl border border-dashed border-gray-300">
                        <div className="flex gap-2 mb-4">
                           <button onClick={() => setNewFile({...newFile, sourceType: 'upload'})} className={`flex-1 py-2 text-sm font-bold rounded ${newFile.sourceType === 'upload' ? 'bg-white shadow' : 'opacity-50'}`}>Upload File</button>
                           <button onClick={() => setNewFile({...newFile, sourceType: 'link'})} className={`flex-1 py-2 text-sm font-bold rounded ${newFile.sourceType === 'link' ? 'bg-white shadow' : 'opacity-50'}`}>External Link</button>
                        </div>
                        
                        {newFile.sourceType === 'upload' ? (
                          <div className="space-y-3">
                             {/* File Picker */}
                             <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-white transition cursor-pointer group">
                             ...
                             {isUploading && (
                               <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                                 <div className={`h-full ${isPizza ? 'bg-pizza-500' : 'bg-lemon-500'} transition-all duration-300`} style={{width: `${uploadProgress}%`}} />
                               </div>
                             )}
                        ...
                        <div className="flex gap-2 mt-4">
                           ...
                           <button 
                              ...
                              className={`px-4 py-2 rounded font-bold text-sm text-white ${isPizza ? 'bg-pizza-600' : 'bg-lemon-600'} disabled:opacity-50`}
                           >
                             Add
                           </button>
                        </div>
                      ...
                      <div className="space-y-2">
                        {newCourse.files?.filter(f => f.name.toLowerCase().includes(fileSearch.toLowerCase())).map((f) => (
                          <div key={f.id} className="flex justify-between items-center bg-white p-3 rounded-lg border shadow-sm text-sm">
                             <div className="flex items-center gap-3">
                                <div className={`p-2 rounded ${isPizza ? 'bg-pizza-100 text-pizza-600' : 'bg-lemon-100 text-lemon-600'}`}>
                 ...
                 {activeTab === 'images' && (
                   <div className="space-y-8">
                      ...
                      <div>
                        ...
                        <input 
                          className="w-full p-3 bg-gray-50 rounded-lg border mb-4"
                          placeholder="https://..."
                      ...
                      <div>
                        ...
                        <div className="mb-4">
                           <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-white transition cursor-pointer group mb-2">
                           ...
                           <input 
                              className="w-full p-3 bg-gray-50 rounded-lg border text-sm"
                      ...
                      <div>
                         ...
                         <div className="grid grid-cols-3 gap-2 mb-4">
                            ...
                            <div className="relative aspect-square bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:bg-white hover:border-gray-400 transition cursor-pointer">
               ...
               <div className="p-6 border-t bg-gray-50 flex gap-3 shrink-0">
                  <button onClick={() => setShowAddModal(false)} className="flex-1 py-3 font-bold text-gray-500 bg-white border rounded-xl hover:bg-gray-100">Cancel</button>
                  <button onClick={handleSaveCourse} className={`flex-1 py-3 font-bold text-white rounded-xl shadow-lg ${isPizza ? 'bg-pizza-600 hover:bg-pizza-700' : 'bg-lemon-600 hover:bg-lemon-700'}`}>
